...
kappa1 <- function(datHH, coeff, zeta, w_c){
  ...
  datHH$AsAr1 <- datHH$As1*datHH$Ar1
  datHH$AsArHH1 <- datHH$As1*datHH$Ar1*datHH$x12H
  datHH$trf1 <- cos(pi*(datHH$x11 + datHH$x11r))
  datHH$trf2 <- (datHH$x12H)^3
  
  
  pre_order1 <- c("trf1",  "trf2",  "As1", "Asx1", "Ar1", "Arx1","AsAr1", "AsArHH1")
...
for (i in 1:r) {
  ...
  trtfree <-   0.5*cos(pi*(datwide1$x11 + datwide1$x11r)) + 0.15* exp(datwide2$x21 + datwide2$x21r) + 0.2*(datwide1$x12 + datwide1$x12r)^3
  ...
  # Pmar <-  expit(-1 + 1.15*exp(x1) - 0.5*x2)
  alpha1 <- glm(A1~ x11 + x12, family = 'binomial', dataInd)
  ...
  theta.desS1 <- model.matrix( ~ z1, data=dataInd)
  estORS1 <- binomial.twostage(alpha1, data = dataInd, var.link = 1,
                               clusters = dataInd$hh,theta.des=theta.desS1)
  estORS1 <- exp(estORS1$theta[1] + estORS1$theta[2] * zz1$x)
  tranA <- tranAA(dataInd$A1[indS], dataInd$A1[indR])
  
  opwS1 <- weights(MarS1, MarR1, estORS1, tranA)$op
  ...
  for (ii in 1:15) {
    ...
    #cos(pi*(datwide1$x11 + datwide1$x11r))  + 0.5*exp(datwide2$x21 + datwide2$x21r) + 0.2*(datwide1$x12 + datwide1$x12r)^3 
    Qclm2 <- clm(QOrdY ~ cos(pi*(x11 + x11r)) + x12H^3 + As1 + I(As1*x11) + Ar1 + I(Ar1*x11r) + I(As1 * Ar1) + I(As1 * Ar1 *x12H), data = datHH1)
    ...
    clmS1 <- clm(OrdY ~ cos(pi*(x11 + x11r)) + x12H^3 + As1 + I(As1*x11) + Ar1 + I(Ar1*x11r) + I(As1 * Ar1) + I(As1 * Ar1 *x12H) , weights = opwS1, data = datHH1)
    ...
    #######################
    clm2 <- clm(OrdY ~ cos(pi*(x11 + x11r)) + x12H^3 + As1 + I(As1*x11) + Ar1 + I(Ar1*x11r) + I(As1 * Ar1) + I(As1 * Ar1 *x12H), weights = w_dS1, data = datHH1)
